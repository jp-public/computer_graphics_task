...
float springStrs[] = { 0.95f, 1.1f, 0.95f, 1.1f, 0.96f, 1.1f };
#endif
	...
	if (x < w && y < h) // Make sure we're inside bounds
	{
...
#ifdef SOLUTION
			vec3 diff = state[end_i].pos - pos;
			float len = length(diff);
			float springLen = scale * springLengths[z / 4];

			if(broken[i * 12 + z] == 0)
				forces[SIZE_Z * gl_LocalInvocationID.x + z] = diff / len * (len - springLen) * springK - vel * .1f;

			float minDistToCorner = min(y, min(x, w - x)) * .5 + min(end_y, min(end_x, w - end_x)) * .5;
			float breakingMult = 1.0 + 3.0 / (1.5 + minDistToCorner * minDistToCorner);
			float springStr = springBreakThreshold * breakingMult * springStrs[z/2];

			if (len / springLen > springStr)
				broken[i * 12 + z] = 1;
#endif
		}
		...
		if (z == 0)
		{
			...
			for (int j = 0; j < SIZE_Z; ++j)
				acceleration += forces[SIZE_Z * gl_LocalInvocationID.x + j];

			//collide with sphere
			if (length((bulletPos.xyz - pos)) < bulletR)
			{
				vec3 n = normalize(pos - bulletPos.xyz);
				state[i].pos = bulletPos.xyz + n * bulletR * 1.01f;
				vel = vel - bulletVel.xyz;
				vec3 newVel = vel + 2.0 * n * abs(dot(vel, n));
				acceleration += (newVel - vel) / dt;
			}

			if (i == 0 || i == w - 1)
			{
				// Corners should be rigid
				result[i].pos = vec3(0);
				result[i].vel = vec3(0);
			}
			else
			{
				// Write results for other particles
				result[i].pos = vel;
				result[i].vel = acceleration;
			}
#endif
		}
	}
