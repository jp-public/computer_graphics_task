...
App::App(void)
	...
	tearing_toggle_(false),
	tearing_mode_changed_(false),
	wind_toggle_(false),
	wind_mode_changed_(false),
	...
	initial_implicit_(false)
{
	...
	common_ctrl_.addToggle(&tearing_toggle_,FW_KEY_L, "Toggle cloth tearing (L)", &tearing_mode_changed_);
	common_ctrl_.addToggle(&wind_toggle_, FW_KEY_K, "Toggle wind (K)", &wind_mode_changed_);
#ifdef COMPUTE_CLOTH_MODULE
...
bool App::handleEvent(const Window::Event& ev) {
	...
	if (tearing_mode_changed_) {
		common_ctrl_.message(tearing_toggle_ ? "Cloth tearing on" : "Cloth tearing off");
		tearing_mode_changed_ = false;
	}

	if (wind_mode_changed_) {
		common_ctrl_.message(wind_toggle_ ? "Wind on" : "Wind off");
		wind_mode_changed_ = false;
	}
...
//#endif
	...
	if (ev.type == Window::EventType_Paint)
	{
		...
		for (int i = 0; i < steps_per_update_; ++i) {
			if (ps_type_ == CLOTH_SYSTEM && tearing_toggle_) {
				cloth_system_.removeSprings(ps_->state());
			}
			if (ps_type_ == CLOTH_SYSTEM && wind_toggle_) {
				cloth_system_.wind_ = true;
			} else {
				cloth_system_.wind_ = false;
			}
			switch (integrator_) {
